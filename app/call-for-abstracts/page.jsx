export default function Page() {
  return (
    <div>
      <h1>Call for Abstracts</h1>

      <p>
        The Organising Committee of the Southeast Asia Catalysis Conference 2026
        (SACC 2026) invites the submission of abstracts for oral and poster
        presentations.
      </p>

      <h2>Important Dates</h2>
      <ul>
        <li><strong>Abstract submission deadline:</strong> 30 September 2026</li>
        <li><strong>Notification of acceptance:</strong> Date/Month  2026</li>
        <li><strong>Conference dates:</strong> 10–11 December 2026</li>
      </ul>

      <h2>Scope and Topics</h2>
      <p>Topics of interest include, but are not limited to:</p>
      <ul>
        <li>TBA</li>
        <li>TBA</li>
        <li>TBA</li>
        <li>TBA</li>
        <li>TBA</li>
      </ul>

      <h2>Submission Guidelines</h2>
      <ul>
        <li>Abstracts must be written in English</li>
        <li>Maximum length: <strong>one page</strong></li>
        <li>Oral and poster presentations are welcome</li>
      </ul>

      <h2>Submission Method</h2>
      <p>
        TBC
      </p>
    </div>
  );
}
